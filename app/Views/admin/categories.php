...
<body class="bg-background text-on-surface selection:bg-primary/30 min-h-screen">
    ...
    <header
        ...
        <form method="GET" action="" class="flex items-center bg-surface-container-high/50 rounded-full px-2 py-2 w-96 ghost-border relative">
            <button type="submit" class="text-secondary hover:text-primary transition-colors ml-1">
                <span class="material-symbols-outlined" data-icon="search">search</span>
            </button>
            <input
                name="search" id="searchInput"
                class="bg-transparent border-none text-sm focus:ring-0 text-on-surface w-full placeholder:text-outline pr-8"
                placeholder="Search categories..." type="text" value="<?php echo $_GET['search'] ?? ''; ?>">
            <?php if (!empty($_GET['search'])): ?>
            <a href="?" class="absolute right-3 top-1/2 -translate-y-1/2 text-secondary/40 hover:text-primary">
